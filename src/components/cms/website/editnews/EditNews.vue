<template>
  <div class="form-news-update form-add">
    <panel>
      <h1 slot="header">更新新闻</h1>
      <vform ref="tplForm" id="news-update-form" :actions="actions" :actionUrls="actionUrls"></vform>
    </panel>
  ...
  export default {
    data () {
      return {
        ...
        tabSelected: 1,
        positionShow: false,
        positionCurrent: 'news',
        actionUrls: {
          backupUrl: commonUrls.vuerouter.news.list,
          saveUrl: commonUrls.news.update
        },
        actions: {
          reinit: false,
          init: function (params) {
            ...
            axios.request(initConfig).then(function (response) {
              axios.request(ruleChangeConfig).then(function (responseInner) {
                ...
                global.store.commit('FORM_FAILURE', {id: 'news-update-form', error})
              })
            }.bind(this)).catch(function (error) {
              global.store.commit('FORM_FAILURE', {id: 'news-update-form', error})
            })
