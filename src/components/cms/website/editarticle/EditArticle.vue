<template>
  <div class="form-article-update form-add">
    <panel>
      <h1 slot="header">更新文章</h1>
      <vform ref="tplForm" id="article-update-form" :actions="actions" :actionUrls="actionUrls"></vform>
    </panel>
  ...
  export default {
    data () {
      return {
        ...
        tabSelected: 1,
        positionShow: false,
        positionCurrent: 'article',
        actionUrls: {
          backupUrl: commonUrls.vuerouter.article.list,
          saveUrl: commonUrls.article.update
        },
        actions: {
          reinit: false,
          init: function (params) {
            ...
            axios.request(initConfig).then(function (response) {
              axios.request(ruleChangeConfig).then(function (responseInner) {
                ...
                global.store.commit('FORM_FAILURE', {id: 'article-update-form', error})
              })
            }.bind(this)).catch(function (error) {
              global.store.commit('FORM_FAILURE', {id: 'article-update-form', error})
            })
