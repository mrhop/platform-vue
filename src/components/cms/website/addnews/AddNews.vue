<template>
  <div class="form-news form-detail">
    <panel>
      <h1 slot="header">新增新闻</h1>
      <vform id="news-add-form" :actions="actions" :actionUrls="actionUrls"></vform>
    </panel>
...
<script>
  ...
  export default {
    data () {
      return {
        actionUrls: {
          backupUrl: commonUrls.vuerouter.news.list
        },
        actions: {
          init: function (params) {
            axios.request(ruleChangeConfig).then(function (responseInner) {
              ...
              global.store.commit('FORM_FAILURE', {id: 'news-add-form', error})
            })
          },
          ...
          save: function (params) {
            ...
            newsSaveConfig.data = params.data
            axios.request(newsSaveConfig).then(function (response) {
              global.store.commit('FORM_SAVE_SUCCESS', {
                id: 'news-add-form',
                data: {}
              })
            }).catch(function (error) {
              global.store.commit('FORM_SAVE_FAILURE', {id: 'news-add-form', error})
            })
          }.bind(this)
        },
        blockListActions: {
