<template>
  <div class="event-list">
    <panel>
      <h1 slot="header">活动列表</h1>
      <vtable id="event-list" :editable="true" :actionUrls="actionUrls" :actions="actions"></vtable>
    </panel>
  </div>
</template>
...
<script>
  ...
  export default {
    data () {
      return {
        actionUrls: {
          addUrl: commonUrls.vuerouter.event.add,
          detailUrl: commonUrls.vuerouter.event.detail,
          infoUrl: commonUrls.vuerouter.event.edit,
          deleteUrl: commonUrls.event.delete
        },
        actions: {
          reinit: false,
          list: function (args) {
            ...
            if (init) {
              axios.request(config).then(function (response) {
                axios.request(ruleChangeConfig).then(function (responseInner) {
                  ...
                  global.store.commit('TABLE_FAILURE', {id: 'event-list', error})
                })
              }).catch(function (error) {
                global.store.commit('TABLE_FAILURE', {id: 'event-list', error})
              })
