<template>
  <div class="form-article-detail form-add">
    <panel>
      <h1 slot="header">文章详情</h1>
      <vform ref="tplForm" id="article-detail-form" :actions="actions" :actionUrls="actionUrls"></vform>
    </panel>
    <panel>
          ...
          init: function (params) {
            ...
            axios.request(initConfig).then(function (response) {
              axios.request(ruleChangeConfig).then(function (responseInner) {
                ...
                global.store.commit('FORM_FAILURE', {id: 'article-detail-form', error})
              })
            }.bind(this)).catch(function (error) {
              global.store.commit('FORM_FAILURE', {id: 'article-detail-form', error})
            })
